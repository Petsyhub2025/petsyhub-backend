...
@Injectable()
export class RefreshTokenStrategy extends PassportStrategy(Strategy, 'customer-refresh-token') {
  constructor(
    ...
    super({
      ...
      secretOrKey: appConfig.CUSTOMER_JWT_REFRESH_SECRET,
    });
  }

  async validate(payload: IRefreshTokenPayload) {
    const session = this.refreshTokenStrategyService.validateCustomerSession(payload);
