...
import { CustomLoggerService, ElasticsearchService, IPetBreedModel, ModelNames } from '@instapets-backend/common';
import { PipelineStage, Types } from 'mongoose';

@Injectable()
export class PetBreedIndexHelperService extends BaseIndexHelper {
  ...
  constructor(
    ...
    super(changeStreamTokenUtil, logger, elasticsearchService, petBreedModel);
    this.esSchema = {
      name: {
        properties: {
          en: {
  ...
  async syncCollection(_ids: string[] = []) {
    ...
    await this.paginationService.paginateAggregate(this.petBreedModel, matchStage, this.pipeline, async (docs) => {
