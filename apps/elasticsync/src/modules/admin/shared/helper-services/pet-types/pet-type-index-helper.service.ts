...
import { CustomLoggerService, ElasticsearchService, IPetTypeModel, ModelNames } from '@instapets-backend/common';
import { PipelineStage, Types } from 'mongoose';

@Injectable()
export class PetTypeIndexHelperService extends BaseIndexHelper {
  ...
  constructor(
    ...
    super(changeStreamTokenUtil, logger, elasticsearchService, petTypeModel);
    this.esSchema = {
      name: {
        properties: {
          en: {
  ...
  async syncCollection(_ids: string[] = []) {
    ...
    await this.paginationService.paginateAggregate(this.petTypeModel, matchStage, this.pipeline, async (docs) => {
