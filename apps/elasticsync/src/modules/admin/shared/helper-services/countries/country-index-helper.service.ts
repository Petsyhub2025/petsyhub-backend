...
import { CustomLoggerService, ElasticsearchService, ICountryModel, ModelNames } from '@instapets-backend/common';
import { PipelineStage, Types } from 'mongoose';

@Injectable()
export class CountryIndexHelperService extends BaseIndexHelper {
  ...
  constructor(
    ...
    super(changeStreamTokenUtil, logger, elasticsearchService, countryModel);
    this.esSchema = {
      name: {
        properties: {
          en: {
  ...
  async syncCollection(_ids: string[] = []) {
    ...
    await this.paginationService.paginateAggregate(this.countryModel, matchStage, this.pipeline, async (docs) => {
