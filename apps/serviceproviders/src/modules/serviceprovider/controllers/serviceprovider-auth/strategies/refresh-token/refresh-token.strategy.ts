...
import { AppConfig, CustomError, ErrorType } from '@instapets-backend/common';
...
@Injectable()
export class RefreshTokenStrategy extends PassportStrategy(Strategy, 'serviceprovider-refresh-token') {
  constructor(
    ...
    super({
      ...
      secretOrKey: appConfig.CLINIC_JWT_REFRESH_SECRET,
    });
  }

  async validate(payload: IRefreshTokenPayload) {
    const session = this.refreshTokenStrategyService.validateServiceProviderSession(payload);
