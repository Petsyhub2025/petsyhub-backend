  ...
  async getCountriesSearchData({
  ...
  }: BaseSearchPaginationQuery): Promise<RpcResponse<ISearchResponseData>> {
    {
      const searches = await this.elasticSearchHelperService.matchQuery({
        index: EsModelNames.COUNTRY,
        query: search,
        searchableFields: [CountryEsFieldsEnum.NameEn, CountryEsFieldsEnum.NameAr],
        saytFields: [CountryEsSaytFieldsEnum.NameEn, CountryEsSaytFieldsEnum.NameAr],
        page,
        limit,
        accurateCount: true,
      });
